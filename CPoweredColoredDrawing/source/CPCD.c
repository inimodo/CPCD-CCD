...
void _CPCD_CLEAR(_CPCD_ALIAS* CPCD_CANVAS, _CPCD_ALIASCOLOR	C_COLOR)
{
...
void _CPCD_PROCESSEVENT(_CPCD_ELEMENT_LIST * EL_LIST, _CPCD_DATA_UINT UI_MSG) {
	...
	for (_CPCD_DATA_DEF i_index = 0; i_index < (*EL_LIST).USI_ELEMENTS; i_index++) {
		...
		_CPCD_DATA_DEF i_width = (*EL_LIST).E_ELEMENTS[i_index].USI_WIDTH;
		_CPCD_DATA_DEF i_height = (*EL_LIST).E_ELEMENTS[i_index].USI_HEIGHT;
...
void _CPCD_ELEMENTRENDER(_CPCD_ALIAS* CPCD_CANVAS, _CPCD_ELEMENTSTYLE ES_STYLE, _CPCD_ELEMENT E_ELEMENT) {
	_CPCD_DATA_DEF i_width = E_ELEMENT.S_DISPLAY.USI_WIDTH+ _CPCD_PIXELQUALITY*2, i_height = E_ELEMENT.S_DISPLAY.USI_HEIGHT+ _CPCD_PIXELQUALITY*2;

	for (_CPCD_DATA_DEF i_index = 0; i_index < _CPCD_PIXELQUALITY; i_index++){
		_CPCD_DRAWRECT(CPCD_CANVAS,
			_CPCD_VECADD(E_ELEMENT.V_POSITION, (_CPCD_ALIASVECTOR) { i_index, i_index}),
			_CPCD_VECADD(E_ELEMENT.V_POSITION, (_CPCD_ALIASVECTOR) { i_width-i_index, i_height-i_index}),
			ES_STYLE.C_DIVIDER[i_index],_CPCD_DM_OUTLINE);
	}
	
	_CPCD_DRAWRECT(CPCD_CANVAS, 
		_CPCD_VECADD(E_ELEMENT.V_POSITION, (_CPCD_ALIASVECTOR) { _CPCD_PIXELQUALITY, _CPCD_PIXELQUALITY}),
		_CPCD_VECADD(E_ELEMENT.V_POSITION, (_CPCD_ALIASVECTOR) { i_width - _CPCD_PIXELQUALITY+1, i_height - _CPCD_PIXELQUALITY+1}),
		ES_STYLE.C_MIDDLE, _CPCD_DM_FILLED);

	for (_CPCD_DATA_DEF i_x = 0; i_x < E_ELEMENT.S_DISPLAY.USI_WIDTH; i_x++)
	{
		for (_CPCD_DATA_DEF i_y = 0; i_y < E_ELEMENT.S_DISPLAY.USI_HEIGHT; i_y++)
		{
			_CPCD_SETPIXEL(CPCD_CANVAS, 
				_CPCD_VECADD(E_ELEMENT.V_POSITION, (_CPCD_ALIASVECTOR) { i_x+ _CPCD_PIXELQUALITY, i_y+ _CPCD_PIXELQUALITY}), 
				_CPCD_GETPIXEL(&(E_ELEMENT.S_DISPLAY),(_CPCD_ALIASVECTOR) { i_x , i_y }));
		}
	}
}

...
void _CPCD_VG_PUSHLINE(_CPCD_VECGRAPHIC * VG_GRAPHIC, _CPCD_UNIONLINE L_LINE)
{
	...
	if (VG_GRAPHIC->USI_SIZE != 0) {
		...
		for (_CPCD_DATA_DEF i = 0; i < VG_GRAPHIC->USI_SIZE; i++) {
...
void _CPCD_DRAWSTRING(_CPCD_ALIAS* CPCD_CANVAS, _CPCD_DATA_STRING C_TEXT, _CPCD_DATA_DEF	USI_LENGTH, _CPCD_ALIASVECTOR V_POINT, _CPCD_DATA_DEF I_SCALE, _CPCD_ALIASCOLOR	C_COLOR){
	...
	for (_CPCD_DATA_DEF i = 0; i < USI_LENGTH; i++){
		i_cursor = (i*((_CPCD_CHARWIDTH*I_SCALE) + (_CPCD_SPACING* I_SCALE)));
		for (_CPCD_DATA_DEF i_heigth = 0; i_heigth < _CPCD_CHARHEIGTH*I_SCALE; i_heigth++) {
			for (_CPCD_DATA_DEF i_width = 0; i_width < _CPCD_CHARWIDTH*I_SCALE; i_width++){

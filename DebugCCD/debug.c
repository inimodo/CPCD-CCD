...
_CPCD_COLOR c_testcolor = {0,255,0};

#define _P_DEGTORAD 0.01745329f

void _drawsequenze(_CPCD_MAP * c_render, _CPCD_VECTOR v_middle, int i_count, int i_length);
void _draw(_CPCD_MAP * c_render, _CPCD_VECTOR v_middle, int i_count);
...
_CPCD_MAIN{
	...
	_CPCD_CLEAR(&(e_element.S_DISPLAY), (_CPCD_COLOR) { 0, 0, 0 });
	
	

	return 0;
}
_CPCD_COLOR cpushader(_CPCD_COLOR c_pixelvalue, _CPCD_VECTOR v_pixelvector)
{
	return c_pixelvalue;
}
_CPCD_UPDATE{
	_CPCD_VECTOR v_cpos = _CPCD_CURSORPOS();
	printf("%d %d\n", v_cpos.X, v_cpos.Y);
...
void _testedit_idle(_CPCD_SPRITE * s_sprite) {
	_CPCD_CLEAR(s_sprite, (_CPCD_COLOR) { 30, 30, 30 });

}
void _testedit_hover(_CPCD_SPRITE * s_sprite) {
	_CPCD_CLEAR(s_sprite, (_CPCD_COLOR) { 255, 0, 0 });

}
void _testedit_clicked(_CPCD_SPRITE * s_sprite) {
	_CPCD_CLEAR(s_sprite, (_CPCD_COLOR) { 0, 255, 0 });
...
_CPCD_VECTOR v_offsets[4] = { {0,1}, {-1,0}, {0,-1} ,{1,0} };

void _draw(_CPCD_MAP * c_render, _CPCD_VECTOR v_middle, int i_count) {
	_CPCD_VECTOR v_offset = {0,0};
	for (int i_index = 1; i_index < i_count; i_index++)
	{
		v_offset =_CPCD_VECADD(v_offset,
			_CPCD_VECMULTI(
				(_CPCD_VECTOR) { _getfibo(i_index-2) , _getfibo(i_index-2)}, v_offsets[i_index % 4]
			)
		);
...
void _drawsequenze(_CPCD_MAP * c_render, _CPCD_VECTOR v_middle, int i_count, int i_length) {
	_CPCD_VECTOR v_outpost,v_first,v_last;
	for (int i_index = 0; i_index <= 90; i_index++)
	{
		...
		_CPCD_DRAWLINE(c_render, v_last, v_outpost, (_CPCD_COLOR) { 255, 255*(i_count%2), 255 });

		if (i_index == 0)v_first = v_outpost;
		v_last = v_outpost;

	}
	_CPCD_DRAWRECT(c_render, v_first, v_last, (_CPCD_COLOR) { 255, 255 * (i_count % 2), 255 },_CPCD_DM_OUTLINE);
}
